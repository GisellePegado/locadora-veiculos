# DML — Inserção de Dados (Data Manipulation Language)

O arquivo `dml.sql` popula as tabelas com dados fictícios fornecidos pela própria atividade no arquivo **"Trabalho – Populando o Banco de Dados"**. Os dados foram criados pelo professor para simular um cenário real de locadora e permitir a validação das consultas implementadas no `dql.md`.

Todos os `INSERT`s devem ser executados **após** o `ddl.sql`, respeitando a ordem abaixo para evitar violações de chave estrangeira.
...
2. Veiculo      → sem dependências
3. Pagamento    → sem dependências
4. Manutencao   → depende de Veiculo
5. Locacao      → depende de Cliente e Pagamento
6. LocacaoVeiculo → depende de Locacao e Veiculo
```
...
Insere **10 clientes** de diferentes estados do Brasil. Cada cliente tem um `idCliente` único como chave primária.
...
Insere **10 veículos** com modelos, marcas e valores de diária variados. O campo `estado` reflete a situação de cada veículo no momento da carga inicial: disponível, alugado ou em manutenção.
...
Insere **20 pagamentos** referentes às 20 locações. Dezesseis estão com `estado = 'Pago'` e quatro com `estado = 'Pendente'` — essa distinção é usada diretamente nas consultas Q2 e Q4 do `dql.md`.
...
Insere **10 registros** de manutenção distribuídos entre 5 veículos diferentes. Alguns veículos aparecem mais de uma vez (ex: veículo 1 teve troca de óleo e substituição de pneu; veículo 6 teve troca de bateria e filtro de ar no mesmo dia). Esses dados alimentam a consulta Q1 do `dql.md`.
...
Insere **20 locações** ligando clientes aos seus pagamentos, com datas de início e fim variadas entre dezembro de 2024 e janeiro de 2025. Cada locação referencia um `idCliente` e um `idPagamento` já inseridos.
...
Insere **20 registros** associando cada locação a um veículo. Como a chave primária é composta (`idLocacao` + `idVeiculo`), cada par só pode aparecer uma vez — o que garante que o mesmo veículo não seja vinculado duas vezes à mesma locação.
...
INSERT INTO LocacaoVeiculo (idLocacao, idVeiculo) VALUES
    (1,2),  (2,3),  (3,5),  (4,8),  (5,1),
    (6,6),  (7,2),  (8,8),  (9,4),  (10,3),
    (11,9), (12,2), (13,6), (14,10),(15,7),
    (16,5), (17,9), (18,2), (19,10),(20,8);
```
